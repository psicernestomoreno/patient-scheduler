...
function apiBase() {
  return (window.SCHEDULER_API_BASE || "").replace(/\/$/, "");
}
...
function appUrl(path) {
  return `${window.location.origin}${window.SCHEDULER_APP_BASE || ""}${path}`;
}
...
async function loadDashboard() {
  ...
  $$('a[href="/auth/google"]').forEach((link) => {
    link.href = serviceUrl("/auth/google");
  });
  $$('a[href="/logout"]').forEach((link) => {
    link.href = serviceUrl("/logout");
  });
  $$('a[href="/book.html"]').forEach((link) => {
    link.href = appUrl("/book.html");
  });
...
async function submitBooking(event) {
  ...
  try {
    ...
    window.location.href = appUrl(`/thanks.html?${params}`);
  } catch (error) {
    updateBookingSubmitState();
...
async function loadThanks() {
  ...
  $("#bookAnother").href = appUrl(`/book.html?lang=${state.language}`);
  renderOfficeMap();
}
